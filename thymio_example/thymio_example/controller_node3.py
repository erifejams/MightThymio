...
class ControllerNode(Node):
    ...
    def update_callback(self):
        ...
        if self.rightProximity != -1.0 or self.leftProximity != -1.0 or self.centerProximity != -1.0 and min(self.rightProximity, self.leftProximity, self.centerProximity) > 2:
            ...
            cmd_vel = Twist() #stop robot from moving
            self.vel_publisher.publish(cmd_vel) 
            ...
            ##go to a 2m point
            while max(self.rightProximity, self.leftProximity) == 2:
                self.get_logger().info('moving away from wall')
                cmd_vel.linear.x  = 1.0
                cmd_vel.angular.z = 0.0 #just heading straight ahead
                self.vel_publisher.publish(cmd_vel)
